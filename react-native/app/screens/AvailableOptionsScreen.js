...
function AvailableOptionsScreen(props) {
    ...
    const updateDate = async (data) => {
        ...
        res = JSON.parse(res);
        res.push({ name: "Selected Option", title: data })
        await AsyncStorage.setItem('data', JSON.stringify(res));
        props.navigation.navigate("AgreementTypeScreen")
    }
                    ...
                    <ScrollView style={{ backgroundColor: Colors.white, flex: 1, width: '100%' }} >
                        ...
                        {data.map((item, i) => (
                            <View key={i} style={{ width: '100%', justifyContent: 'center', alignItems: 'center', marginTop: RFPercentage(5) }}>
                                <TouchableOpacity onPress={() => updateDate(item.mainTitle)} style={{ width: '80%', flexDirection: 'row', alignItems: "center", justifyContent: 'flex-start' }}>
